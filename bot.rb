...
module Bunto
  class Bot
    ...
    # Remind people to include `bunto env`

    def process_env_check(issue)
      body = issue.body + issue.title
      unless body.include?("Loaded bunto plugins")
        puts "https://github.com/#{SLUG}/issues/#{issue.number} (#{issue.title}) seems to be missing env report"
        body = []
        body << "It seems like you have not included the output of `bunto env`."
        body << "To make it easier for us help you resolve this issue, please update the issue to include the output of `bunto env` :+1:"
        return body.join("\n\n")
      end
      return nil
    end
